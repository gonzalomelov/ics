...
function buildCalendar(calendarAttributes, eventsAttributes) {
  if (!eventsAttributes || _.isEmpty(eventsAttributes)) {
    return buildDefaultCalendar(calendarAttributes);
  } else {
    var file = [
      'BEGIN:VCALENDAR',
      'VERSION:2.0',
      ...
      'METHOD:' + calendarAttributes.method
    ];
    eventsAttributes.forEach(function(attributes) {
      file = file.concat(buildEvent(attributes));
    })
    file = file.concat(['END:VCALENDAR']);
    return _.compact(file).join('\r\n');
  }

  function buildDefaultCalendar(calendarAttributes) {
    var file = [
      'BEGIN:VCALENDAR',
      'VERSION:2.0',
      'CALSCALE:GREGORIAN',
    ...
    if (!attributes || _.isEmpty(attributes)) {
      ...
      return []
        .concat([
          ...
          'DTSTAMP:' + generateDateTimeStamp(),
          formatProperty('SEQUENCE', attributes.sequence),
...
ICS.prototype.buildCalendar = function(calendarAttributes, eventsAttributes) {
  return buildCalendar(calendarAttributes, eventsAttributes);
};
...
ICS.prototype.createEvent = function(attributes, _options_, cb) {
  ...
  ,   file = this.buildCalendar({method: 'REQUEST'}, [attributes])
