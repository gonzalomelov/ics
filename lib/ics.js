...
function buildCalendar(eventsAttributes) {
  if (!eventsAttributes || _.isEmpty(eventsAttributes)) {
    return buildDefaultCalendar();
  } else {
    var file = [
      'BEGIN:VCALENDAR',
      'VERSION:2.0',
...
ICS.prototype.buildCalendar = function(eventsAttributes) {
  return buildCalendar(eventsAttributes);
};
...
ICS.prototype.createEvent = function(attributes, _options_, cb) {
  ...
  ,   file = this.buildCalendar([attributes])
